            ...
            <div className="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
              <div>
                <p className="text-sm text-gray-700">
                  {t('common.showing')} <span className="font-medium">{(((pagination?.page || 1) - 1) * (pagination?.limit || 20)) + 1}</span> {t('common.to')} <span className="font-medium">{Math.min((pagination?.page || 1) * (pagination?.limit || 20), pagination?.total || 0)}</span> {t('common.of')} <span className="font-medium">{pagination?.total || 0}</span> {t('common.results')}
                </p>
              </div>
              <div>
